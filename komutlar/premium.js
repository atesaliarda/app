...
exports.run = async (client, message, args) => {
  ...
  var g = "`o!premium puan` yazarak görebilirsiniz."
  var k = "Premium Aktif Sunucular"
  var l = "`o!premium liste` yazarak görebilirsiniz."
    if(db.has(`dil_${message.guild.id}`) === true) {
        ...
        var g = "You can see by writing `o!premium point`."
        var k = "Premium Active Servers"
        var l = "You can see by writing `o!premium list`."
    }
    const dil = client[s]
    const o = a
...
message.channel.send(embed)
  ...
  if (i === "aç" || i === "aktif") {
    ...
    if (message.author.id !== "290922519769382913") return message.channel.send(`:x: Bunu Sadece Sahibim Kullanabilir !`);
